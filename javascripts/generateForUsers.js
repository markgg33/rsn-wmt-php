...
document.getElementById("generatePdfBtnUsers").addEventListener("click", () => {
  ...
  img.onload = () => {
    ...
    const userName = document.getElementById("loggedInUser").dataset.username;

    if (monthText || userName) {
      doc.setFontSize(11);
      doc.text(`Month: ${monthText}`, 150, 70);
      doc.text(`User: ${userName}`, 150, 85);
    }
    ...
    // Save the PDF
    const filename = `Monthly_Summary_${monthText}_${userName}`.replace(
      /\s+/g,
      "_"
    );
